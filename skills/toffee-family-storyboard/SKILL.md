...
description: Toffee Family 分镜头脚本双Agent闭环检验系统。用于 Toffee Family 儿童剧及同类低龄内容的剧本拆分、对白逐字零删减、镜头合并与景别优化、视角和时长设计、内置灰米色黑线稿表演分镜绘制、AI提示词转换及双Agent量化质检，直至评分通过或强制终止。
---
...
> **视觉分镜硬门禁**：具备图像生成能力时，逐镜分镜图是必交物，不得只交分镜表或提示词。绘制前必须读取 [`references/storyboard-visual-style.md`](references/storyboard-visual-style.md)，并把 `assets/storyboard-style/` 中四张图片作为风格参考；只借用线稿视觉语言，不复制参考图中的角色、场景或构图。
...
| 包含必要生成元素与风格锚点 | 3分 | 缺少景别/角度/光影/色彩或内置线稿风格锚点中的任意一项，每处扣1分 |
...
每次绘制分镜图前，完整读取 [`references/storyboard-visual-style.md`](references/storyboard-visual-style.md)。默认把以下四张图片同时作为风格参考输入；若工具限制参考图数量，至少使用 `reference-01.png` 和 `reference-04.png`：
...
参考图只控制线条、底色、简化程度和表演可读性。Toffee Family 角色设定、用户提供的角色参考图、剧本动作与场景连续性拥有更高优先级。
...
**AI生图提示词**：大特写，平视角度，透明玻璃杯位于画面中央，一缕热气从杯口升起，背景只保留台灯轮廓，暖灰纸面底色，黑色松散动画分镜线稿，允许试探重线，局部淡黄色光点，只画轮廓与关键表演信息，不上色，不做精修插画
...
15. ✅ 所有分镜图保持暖灰底、黑色松散线稿、表演优先、背景极简的内置风格，没有复制参考图角色或漂移为精修插画
...
| 参考风格等于照搬参考图 | 只提取灰米底、黑线稿、表演优先等视觉语言；角色、场景和构图必须来自当前 Toffee Family 剧本 |
...
### 镜头1
[景别], [角度], [构图与角色位置], [决定性瞬间], [必要环境线索], [光影与受控色彩], 暖灰纸面底色，黑色松散动画分镜线稿，允许试探重线，表演优先，背景极简，不上色，不做精修插画
```
