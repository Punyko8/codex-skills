...
description: Toffee Family 分镜头脚本双Agent闭环检验系统。用于 Toffee Family 儿童剧及同类低龄内容的剧本拆分、对白逐字零删减与语速核验、场面调度、表演节拍、镜头合并、交付模式设计、黑白低完成度动画粗草绘制、AI提示词转换及双Agent量化质检，直至评分通过或强制终止。
---
...
> **对白语速门禁**：普通儿童剧对白的实际发声速度尽量保持在每秒 2.0–2.33 个可发音字。高于 2.33 必须延长；低于 2.0 必须说明表演或理解停顿原因。对白发声时长与镜头总时长分开计算。
...
> **导演生产硬门禁**：拆镜前完整读取 [`references/directing-production.md`](references/directing-production.md)，先锁定交付模式并建立场景空间表；对白镜头必须填写表演节拍，输出页面必须建立页面—宫格—镜号—对白 ID 映射。
...
6. 普通对白按可发音字数和实际发声时长计算语速。使用 `scripts/dialogue_timing_check.py` 机械核验；过快项必须为 0，偏慢项必须附带节拍理由。
...
含对白、连续动作或关键反应的镜头，在确定时长后必须建立镜内表演节拍：分别标记对白实际发声时码、说话者动作、倾听者反应、画面焦点和声音剪辑方式。普通对白优先保持每秒 2.0–2.33 个可发音字；镜头时长还要另加动作、反应和停顿。
...
**前置硬门禁**：量化评分只在对白、空间、表演、页面映射和视觉门禁全部通过后进行。对白覆盖率低于 100%，存在任意删句、缩写、同义改写、错序、错配说话者或未处理的过快项，直接打回；总分再高也不得通过。完整门禁见 [`references/directing-production.md`](references/directing-production.md)。
...
graph TD
 ...
 B --> C[提交：分镜表+导演生产表+创作说明]
 C --> D[质检者Agent：四级检查并打分]
 D --> E{总分≥90且所有层级合格?}
 E -->|是| F[输出：最终分镜表+导演生产表+视觉分镜图+AI提示词]
 E -->|否| G{修改轮次≥5?}
...
4. 涉及镜头合并或拆分时，必须重新核验镜号连续、时码无空档/重叠、总时长、空间状态链、页面映射与提示词一一对应
...
1. **最优终止**：全部前置硬门禁通过、总分≥90分且所有层级达到合格线 → 输出最终分镜表 + 导演生产表 + 视觉分镜图 + AI生图提示词
...
3. 使用任务开始时锁定的交付模式、视觉模式和画幅；未指定时默认 `shot-card + gray-rough + 16:9`。
...
13. ✅ 普通对白实际发声速度优先保持每秒 2.0–2.33 个可发音字；无未处理的过快项，偏慢项均有节拍理由
14. ✅ 场景空间表、场面调度、角色屏幕侧、视线、出入口和道具状态链连续
15. ✅ 含对白和关键反应的镜头具有镜内表演节拍，说话者动作与倾听者反应不同时抢焦点
16. ✅ 页面、宫格、镜号和对白 ID 映射完整，无遗漏、重复或错序
17. ✅ 视觉图片数量和组织方式符合已锁定的 `shot-card`、`group-sheet` 或 `hybrid` 模式
18. ✅ 角色锚点和所有分镜图符合已锁定的 `pure-line` 或 `gray-rough` 模式，没有彩色色相、参考图角色复制或成稿级精修漂移
...
| 镜号 | 时码 | 景别 | 视角角度 | 镜头运动 | 时长 | 画面内容 | 声音 | 核心目标 | 场面调度 | 连续性锚点 | 道具状态 | 剪辑点 |
|------|------|------|---------|---------|------|---------|------|---------|----------|------------|----------|--------|
...
| 对白ID | 原剧本说话者 | 原剧本对白原文 | 可发音字数 | 实际发声时长 | 字/秒 | 分镜镜号 | 核验 |
|--------|--------------|----------------|------------|--------------|-------|----------|------|
| D001 | ... | ... | ... | ... | ... | ... | 逐字一致；语速合格 |
